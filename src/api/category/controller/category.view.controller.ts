...
export default class CategoryViewController {
  ...
  async userCategoryDetailPage(req: Request, res: Response, next: NextFunction) {
    const category = await this._categoryService.getCategory();
    const posts = await this._postService.findByCategoryId(req.params.categoryId);
    const likedPosts = posts.filter((post) => post.likes.some((like) => like.user.id === req.user.userId));
    const userId = req.user.userId;
    res.render("client/category/userCategoryDetail", { category, posts, likedPosts, userId });
  }
}
